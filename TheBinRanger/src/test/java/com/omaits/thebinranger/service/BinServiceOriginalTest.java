...
class BinServiceOriginalTest {

    List<BinRange> smallBinRangeList;
    List<BinRange> largeBinRangeList;
    BinRange binRange;

    BinServiceOriginal binServiceOriginal;
    ...
    void get_57124101_fiveReturned() {
        binServiceOriginal = new BinServiceOriginal(smallBinRangeList);
        assertEquals("five", binServiceOriginal.get("57124101000000"));
    }

    @Test
    void get_27124101_twoReturned() {
        binServiceOriginal = new BinServiceOriginal(smallBinRangeList);
        assertEquals("two", binServiceOriginal.get("27124101000000"));
    }

    @Test
    void get_37124101_37124101Returned() {
        binServiceOriginal = new BinServiceOriginal(smallBinRangeList);
        assertEquals("three", binServiceOriginal.get("37124101000000"));
    }

    @Test
    void get_bigBinRangeList_lastItemReturned() {
        binServiceOriginal = new BinServiceOriginal(largeBinRangeList);
        assertEquals(largeBinRangeList.get(largeBinRangeList.size() - 1).cardType, binServiceOriginal.get(largeBinRangeList.get(largeBinRangeList.size() -1).start + 1));
    }
